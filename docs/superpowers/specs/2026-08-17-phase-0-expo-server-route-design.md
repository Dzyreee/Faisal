...
6. `app/index.tsx` — one screen, fetches the route on mount via a **relative**
   URL, renders the raw JSON response.

7. `CLAUDE.md` updated (see below).
...
| 4 | Secret containment | `npx expo export --platform ios`, then grep `dist/` for `hunter2` **and** `FAKE_KEY` — both must return zero hits | the secret actually shipping |

Check 4 greps for the **value**, not only the key name. The value is what leaks.
Grepping our own source proves nothing, since we never write `FAKE_KEY` in app
code and it would pass trivially.
...
2. `phase 0: expo skeleton + server-side API route` — scaffold, route, screen,
   test, `.env.example`, and the `CLAUDE.md` update, which is part of the phase
